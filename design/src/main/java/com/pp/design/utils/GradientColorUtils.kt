...
internal fun Color.lightenColor(value: Float): Color {
    val hsl = toHSL()
    hsl[2] += value
...
internal fun Color.darkenColor(value: Float): Color {
    val hsl = toHSL()
    hsl[2] -= value
    hsl[2] = 0f.coerceAtLeast(hsl[2].coerceAtMost(1f))
    return hslToColor(hsl)
...
internal fun Color.isLightColor(hsl: FloatArray = FloatArray(3)): Boolean {
    ColorUtils.RGBToHSL((red * 255f).roundToInt(), (green * 255f).roundToInt(), (blue * 255f).roundToInt(), hsl)
    return hsl[2] > .5f
}

internal fun Color.isDarkColor(hsl: FloatArray = FloatArray(3)): Boolean {
    ColorUtils.RGBToHSL((red * 255f).roundToInt(), (green * 255f).roundToInt(), (blue * 255f).roundToInt(), hsl)
    return hsl[2] < .5f
}

internal fun generateGradientFeel(baseColor: Color, lightenFactor: Float = 0.3f): Brush {
...
internal fun Color.getContentColor() =
    when {
        isDarkColor() -> {
            lightenColor(.65f)
        }
